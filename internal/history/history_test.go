...
func TestStore_NotFoundErrorFormat(t *testing.T) {
	t.Parallel()
	...
	// Task IDs generated by the agent have a "task-" prefix
	// The error message should not create confusing output like "task task-abc123"
	taskID := "task-abc123"
	_, err = store.Get(taskID)
	require.Error(t, err)

	// The error message should be clear and not duplicate "task"
	// Bad: "task task-abc123 not found in history" (confusing)
	// Good: "task-abc123 not found in history" (clear)
	errMsg := err.Error()
	require.NotContains(t, errMsg, "task task-", "error message should not duplicate 'task' prefix")
	require.Contains(t, errMsg, taskID)

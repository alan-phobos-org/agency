#!/bin/bash
# Start agency: web director + claude agent
...
if [ ! -f "$PROJECT_ROOT/bin/ag-director-web" ] || [ ! -f "$PROJECT_ROOT/bin/ag-agent-claude" ]; then
...
"$PROJECT_ROOT/bin/ag-director-web" -port "$WEB_PORT" -env "$PROJECT_ROOT/.env" > "$PID_DIR/director.log" 2>&1 &
DIRECTOR_PID=$!
...
echo -n "Waiting for director..."
if ! wait_for_status "Web director" "https://localhost:$WEB_PORT/status" "$DIRECTOR_PID"; then
    echo " failed. Check $PID_DIR/director.log"
...
echo ""
echo "Logs:"
echo "  Director: $PID_DIR/director.log"
